    ...
    while next_time <= end_date:
        try:
                    ...
                    total_profit[loc_id].update(
                        {
                            dept_id: {
                                ...
                                'total_sales_tax': float(
                                    product.get('Thing_Intangible_StructuredValue_PriceSpecification_salesTax', 0)),
            ...
            for location in total_profit:
                if location in transaction_count_data:
                    for dept in total_profit[location]:
                        resp.append(
                            {
                                'PK': f"{start_date.strftime('AGG#%Y')}",
                                'SK': f"{prev_time.strftime('%Y-%m-%d %H:%M:%S')}-{next_time.strftime('%H:%M:%S')}#{str(location)}",
                                'total_profit': str(round(total_profit[location][dept]['total_profit'], 2)),
                                'total_profit_margin': str(
                                    round(total_profit[location][dept]['total_profit_margin'], 2)),
                                'total_sales_tax': str(round(total_profit[location][dept]['total_sales_tax'], 2)),
                                'total_quantity': int(total_profit[location][dept]['total_quantity']),
                                'total_items': int(total_profit[location][dept]['total_items']),
                                'loc_id': int(location),
                                'dept_id': int(dept),
                    ...
                    # Handle the case when the location key does not exist in transaction_count_data
                    # You can choose to skip or perform any other necessary action
                    pass
